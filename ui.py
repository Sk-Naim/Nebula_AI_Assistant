...
class HudCanvas(QWidget):
    ...
    def _update_tick(self):
        ...
        # Only update HUD animations if NOT speaking
    ...
    def paintEvent(self, _):
        ...
        # Only draw animated circles if NOT speaking
        if not self._speaking:
            # grid dots
            ...
            for idx, (r_frac, w_r, arc_l, gap) in enumerate([(0.48, 3, 115, 78), (0.40, 2, 78, 55), (0.32, 1, 56, 40)]):
                ...
                col    = qcol(C.MUTED_C if self.muted else C.PRI, 150)
            ...
            p.setPen(QPen(qcol(C.PRI, 150), 2.5))
            p.drawArc(srect, int(self._scan * 16), 45 * 16)
            p.setPen(QPen(qcol(C.ACC, 100), 1.5))
            p.drawArc(srect, int(self._scan2 * 16), 45 * 16)
            ...
            # Central orb pulse
            orb_r = int(fw * 0.25 * (1.0 + 0.05 * math.sin(self._tick * 0.1)))
            oc = (120, 0, 180) # purple
            for i in range(5, 0, -1):
                r2 = int(orb_r * i / 5)
                a = max(0, min(255, int(150 * (i/5))))
                p.setBrush(QBrush(QColor(oc[0], oc[1], oc[2], a)))
                p.setPen(Qt.PenStyle.NoPen)
                p.drawEllipse(QRectF(cx - r2, cy - r2, r2 * 2, r2 * 2))

            # Nebula AI Text in center of orb
            p.setPen(QPen(qcol(C.WHITE, 220), 1))
            p.setFont(QFont("Courier New", 14, QFont.Weight.Bold))
            p.drawText(QRectF(cx - 100, cy - 15, 200, 30), Qt.AlignmentFlag.AlignCenter, "Nebula AI")
        ...
        elif self._speaking: txt, col = "●  SPEAKING", qcol(C.PRI)
        elif self.state == "THINKING": txt, col = "◈  THINKING", qcol(C.ACC)
        elif self.state == "LISTENING": txt, col = "●  LISTENING", qcol(C.GREEN)
        else: txt, col = f"●  {self.state}", qcol(C.PRI)

        p.setPen(QPen(col, 1))
        p.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        p.drawText(QRectF(0, sy, W, 30), Qt.AlignmentFlag.AlignCenter, txt)

        # waveform (always show)
        wy = sy + 35
        N, bw = 40, 8
        wx0 = (W - N * bw) / 2
        for i in range(N):
            if self.muted: hgt, cl = 2, qcol(C.MUTED_C)
            elif self._speaking: hgt, cl = random.randint(3, 25), qcol(C.PRI)
...
class LogWidget(QTextEdit):
    ...
    def _next(self):
        ...
        elif tl.startswith("nebula:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for NEBULA AI", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure NEBULA AI. before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class MainWindow(QMainWindow):
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("NEBULA AI", C.PRI_DIM))
        lay.addStretch()
        ...
        mid.addWidget(title)
        sub = QLabel("Neural Engineered Being for Universal Life Assistance")
    ...
    def _on_file_selected(self, path: str):
        ...
        self._file_hint.setText(f"{icon}  {p.name}  ·  {size}  ·  Tell NEBULA AI what to do with it")
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._log.append_log(f"SYS: Initialised. OS={os_name.upper()}. NEBULA AI online.")
