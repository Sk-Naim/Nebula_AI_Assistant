...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".nebula_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".nebula_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using NEBULA AI profile")
                nebula = str(Path.home() / ".nebula_profiles" / "firefox_nebula")
                Path(nebula).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(nebula, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".nebula_profiles" / "safari")
        ...
        try:
            ...
            self._context = await asyncio.wait_for(
                engine_obj.launch_persistent_context(profile, **kwargs),
                timeout=10.0
            )
            await asyncio.sleep(0.5) 
            self._page = await self._context.new_page()
            ...
            print(f"[Browser] ⚠️  Real profile failed or timed out for {label}: {e}")
            # If the browser process was spawned but we timed out connecting, we might need to rely on the fallback.

        nebula_profile = str(Path.home() / ".nebula_profiles" / self.browser_name)
        Path(nebula_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with NEBULA AI profile: {nebula_profile}")

        try:
            _clean_profile(nebula_profile)
            self._context = await engine_obj.launch_persistent_context(nebula_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with NEBULA AI profile")
    ...
    async def _get_page(self) -> Page:
        ...
        if self._context:
            try:
                ...
                print("[Browser] 🎤 Microphone permission granted.")
            except Exception as pe:
                print(f"[Browser] ⚠️ Failed to grant mic permission: {pe}")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "nebula_screenshot.png")
    ...
    async def tune_guitar_flow(self, player=None) -> str:
        try:
            # 1. Navigate to the tuner website
            await self.go_to("https://guitartuna.com/online-guitar-tuner")
            # 2. Wait 2 seconds for elements to render
            await asyncio.sleep(2)
            # Ensure top view before clicking
            page = await self._get_page()
            try:
                await page.evaluate("window.scrollTo(0, 0)")
            except Exception:
                pass
            # 3. Click "Tap to tune" programmatically to prevent scrolling down
            locator = page.get_by_text("Tap to tune", exact=False).first
            await locator.dispatch_event("click")
            # Ensure top view after clicking
            try:
                await page.evaluate("window.scrollTo(0, 0)")
            except Exception:
                pass
            return "Opened guitar tuner and clicked 'Tap to tune' successfully in top view."
        except Exception as e:
            return f"Error opening tuner flow: {e}"

    async def swiggy_order_flow(self, player=None) -> str:
        try:
            # 1. Open the specific link provided by the user
            await self.go_to("https://www.swiggy.com/city/bardhaman/order-online")
            
            # 2. Wait 3 seconds
            await asyncio.sleep(3.0)

            # 3. Open the checkout link directly
            await self.go_to("https://www.swiggy.com/checkout")
            
            # 4. Click 'DELIVER HERE' on the first address
            try:
                page = await self._get_page()
                await page.locator("text=DELIVER HERE").first.click(timeout=3500)
                await asyncio.sleep(1.5)
            except Exception as e:
                print(f"[Swiggy] Could not click DELIVER HERE: {e}")
            
            return (
                "SUCCESS. Tell the user exactly this word for word, sounding very natural and conversational in Hindi: "
                "'Maine Golden Corn Pizza aapke cart mein add kar diya hai, aur aapka order doorstep par deliver hone ke liye bilkul ready hai! "
                "Payment ke liye aap kya choose karenge, Cash on Delivery ya PhonePe?'"
            )
        except Exception as e:
            return f"Error opening swiggy flow: {e}"

    async def swiggy_pay_phonepe(self, player=None) -> str:
        try:
            page = await self._get_page()
            if not page:
                return "No browser open"
            
            async def force_click(locator_text, exact=True):
                try:
                    # First try exact match
                    elements = page.get_by_text(locator_text, exact=exact)
                    ...
                    for i in range(count):
                        ...
                        if await el.is_visible():
                            ...
                            if box and box["width"] > 0 and box["height"] > 0:
                                ...
                                await page.mouse.up()
                                print(f"[Swiggy] Force clicked '{locator_text}' (exact={exact})")
                                return True
                                
                    # If exact fails, try non-exact substring match
                    if exact:
                        return await force_click(locator_text, exact=False)
                except Exception as e:
                    print(f"[Swiggy] Force click failed for {locator_text}: {e}")
                
                print(f"[Swiggy] Could not find visible element for '{locator_text}'")
                return False
            
            # 1. Click PROCEED TO PAY on the checkout page
            await force_click("PROCEED TO PAY")
            await asyncio.sleep(3.0) # Wait for page transition

            # 2. Click Wallets
            await force_click("Wallets")
            await asyncio.sleep(1.5)

            # 3. Click PhonePe option
            await force_click("PhonePe", exact=False)
            await asyncio.sleep(1.5)
                
            # 4. Click the final Pay button
            await force_click("Pay ₹", exact=False)
            await force_click("Pay", exact=True)
            
            return (
                "SUCCESS. Tell the user exactly this in Hindi: "
                "'Aapke mobile par PhonePe payment request confirmation bheja gaya hai. Kripya ise check karein.'"
            )
        except Exception as e:
            return f"Error in phonepe payment: {e}"
...
def browser_control(
    ...
    try:
        ...
        elif action == "tune_guitar":
            result = sess.run(sess.tune_guitar_flow(player))
        elif action == "swiggy_order":
            result = sess.run(sess.swiggy_order_flow(player))
        elif action == "swiggy_pay_phonepe":
            result = sess.run(sess.swiggy_pay_phonepe(player))
        elif action == "search":
