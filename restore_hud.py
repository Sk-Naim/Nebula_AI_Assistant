...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, parent=None):
        ...
        self._face_px = None  # No face, just UI circles
    ...
    def _update_tick(self):
        ...
        for i in range(len(self._pulses)-1, -1, -1):
            self._pulses[i] += 4.0
            if self._pulses[i] > self.width() * 0.4:
                self._pulses.pop(i)
    ...
    def paintEvent(self, _):
        ...
        oc    = (200, 0, 50) if self.muted else (0, 150, 200) # cyan/blue
        for i in range(8, 0, -1):
            r2  = int(orb_r * i / 8)
            frc = i / 8
            a   = max(0, min(255, int(self._halo * 1.1 * frc) + 20)) # Always slightly visible
            p.setBrush(QBrush(QColor(int(oc[0]*frc), int(oc[1]*frc), int(oc[2]*frc), a)))
            p.setPen(Qt.PenStyle.NoPen)
            p.drawEllipse(QRectF(cx - r2, cy - r2, r2 * 2, r2 * 2))
        p.setPen(QPen(qcol(C.PRI, min(255, int(self._halo * 2) + 100)), 1))
        p.setFont(QFont("Courier New", 13, QFont.Weight.Bold))
        p.drawText(QRectF(cx - 80, cy - 14, 160, 28), Qt.AlignmentFlag.AlignCenter, "NEBULA AI")
...
pattern = re.compile(r"class AvatarCanvas\(QWidget\):.*?(?=^class MetricBar)", re.MULTILINE | re.DOTALL)
new_content = pattern.sub(hud_canvas_code + "\n\n", content)

new_content = new_content.replace("self.hud = AvatarCanvas(face_path)", "self.hud = HudCanvas(face_path)")

with open(filepath, "w") as f:
    f.write(new_content)
print("Reverted to HudCanvas without face")
